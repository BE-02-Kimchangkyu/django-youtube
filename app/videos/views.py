...
class VideoList(APIView):
    def get(self, request):
        ...
        serializer = VideoListSerializer(videos, many=True) # 쿼리셋 한 데이터가 2개 이상일 때.
...
class VideoDetail(APIView):
    ...
    def put(self, request, pk):
        video = Video.objects.get(pk=pk) # db에서 불러 온 데이터
    ...
    def delete(self, request, pk):
        video = Video.objects.get(pk=pk) # dv에서 불러온 데이터
        video.delete()
